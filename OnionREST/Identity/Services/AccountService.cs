...
namespace Identity.Services
{
    public class AccountService : IAccountService
    {
        ...
        public async Task<Response<AuthenticationResponse>> AutenticateAsycn(AuthenticationRequest request, string ipAddress)
        {
            var usuario = await _userManager.FindByEmailAsync(request.Email);
            if (usuario == null)
            {
                throw new ApiException($"esta cuenta no esta registrada {request.Email}");
            }
        ...
        public async Task<Response<string>> RegisterAsycn(RegisterRequest request, string origin)
        {
            var usuarioConElMismoUserName = await _userManager.FindByNameAsync(request.UserName);
            if (usuarioConElMismoUserName != null)
            {
                throw new ApiException($"Nombre de usuario {request.UserName} ya esta registrado");
            }
            ...
            {
                throw new ApiException($"Email {request.UserName} ya esta registrado");
            }
            else 
            {
